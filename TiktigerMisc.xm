...
%end
%hook UIApplication
- (void)openURL:(NSURL *)url options:(NSDictionary *)options completionHandler:(void (^)(BOOL))handler{ %orig(url, options, handler); }
%end
...
- (void)sendActionsForControlEvents:(UIControlEvents)events{ if(TTBool(@"liveButtonAction")&&[self.accessibilityLabel.lowercaseString containsString:@"live"]) return; %orig(events); }
%end
